    ...
    <v-alert class="mt-2 ml-2" dense border="left" colored-border color="green accent-4" elevation="0">
      <v-card elevation="0">
        <v-row>
          ...
          <v-col>
            <v-autocomplete v-model="step.MouseType" :items="mouselist" auto-select-first dense class="pt-2">
              <template v-slot:prepend-inner>
                <p class="indigo--text text--accent-4 my-1">点击类型:</p>
              </template>
            </v-autocomplete>
          </v-col>
        </v-row>
...
export default {
  ...
  data() {
    return {
      // 操作列表
      optlist: ['Mouse.Click', 'Mouse.Move'],
      // mouse 操作列表
      mouselist: ['ClickLeft', 'ClickRight', 'DoubleLeft', 'DoubleRight'],
      // boolean 列表
