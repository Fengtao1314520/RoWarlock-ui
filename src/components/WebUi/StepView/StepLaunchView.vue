    ...
    <v-alert class="mt-2 ml-2" dense border="left" colored-border color="green accent-4" elevation="0">
      <v-card elevation="0">
        <v-row>
          ...
          <v-col cols="2">
            <v-autocomplete v-model="step.BrowserType" :items="browserlist" dense auto-select-first class="pt-2">
              <template v-slot:label>
                <p class="indigo--text text--accent-4 my-1">浏览器:</p>
              </template>
            </v-autocomplete>
          </v-col>
          <v-col cols="2">
            <v-autocomplete v-model="step.Run64Bit" :items="booleanlist" auto-select-first dense class="pt-2">
              <template v-slot:label>
                <p class="indigo--text text--accent-4 my-1">运行64位:</p>
              </template>
            </v-autocomplete>
          </v-col>
        </v-row>
...
export default {
  ...
  data() {
    return {
      optlist: ['Launch.Browser'],
      // 浏览器 列表
      browserlist: ['Chrome', 'ChromeHeadless'],
      // boolean 列表
